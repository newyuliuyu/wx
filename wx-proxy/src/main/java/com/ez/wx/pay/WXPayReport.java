...
public class WXPayReport {
    ...
    private WXPayReport(final WXPayConfig config) {
        ...
        if (config.shouldAutoReport()) {
            ...
            // 添加处理线程
            executorService = new ThreadPoolExecutor(config.getReportWorkerNum(), config.getReportWorkerNum(),
                    0L, TimeUnit.MILLISECONDS,
                    new LinkedBlockingQueue<Runnable>(), this::createThread);
...
//            });
            ...
            for (int i = 0; i < config.getReportWorkerNum(); ++i) {
                executorService.execute(this::uploadReport);
            }
//            for (int i = 0; i < config.getReportWorkerNum(); ++i) {
...
//            }
    ...
    public void report(String uuid, long elapsedTimeMillis,
        ...
        if (data != null && config.shouldAutoReport()) {
